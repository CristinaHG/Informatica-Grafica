...
void draw_objects()
{
    ...
    //copiando vertices
    solidoply->ver =  (vertices*) malloc(solidoply->n_v*sizeof(vertices)); //new solido.ver[4005];

    for(int i=0;i<solidoply->n_v;i++){
        for(int j=0; j<3;j++){
            solidoply->ver[i].coord[j]=v_ply.at(3*i+j);
        }
    }
...
//    }
    //generar por rev
    generaPorRevolucion(solidoply, 6);
